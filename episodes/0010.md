...
In this episode, everyone brings their favorite, must-run self-hosted apps, with hometown favorites like Home Assistant, Plex, and Jellyfin explicitly off the table.
...
After that, Adam unpacks his own picks: Pangolin, which he describes as self-hosted Cloudflare Tunnels with built-in authentication and reverse proxying, and Pi-hole for network-wide ad blocking. That leads into a debate with Stephen and Geoff about Pangolin's shift toward a business-backed license, and a side-by-side of Pi-hole, Technitium, and NextDNS for DNS filtering.

Finally, Geoff relays a few of Alex's picks along of Karakeep (formerly Hoarder) for archiving and tagging saved links, and Paperless-ngx for scanning and organizing documents.  Geoff also shares some honorable mentions of Dawarich for location history recording and tracking and Termix for SSH'ing on the go.
