...
5. Verify access at `https://metrics.komodo-gecko.ts.net` from the tailnet without an application login, HTTPS through Tailscale Serve, no published container port, and no public Funnel exposure. Verify the Dashy link and preservation of all existing Dashy sections.
