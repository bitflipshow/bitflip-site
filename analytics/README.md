...
The overview and archive show metric summaries for the episodes in view (including the median and best completed window), one chronological comparison chart, and a dense episode table. The chart shows audio downloads by default with a dashed median line; lifetime views can switch to public YouTube views or both, and each selection rescales the axis. Windows still collecting or with partial history are hatched in the chart and marked `*` or `†` in the table. Table headings sort highest-first (`?sort=24h|7d|30d|lifetime|youtube|spotify`); a sorted archive ranks every match before paging. Episode pages compare the episode's 24h/7d/30d downloads with the catalogue median of completed windows. The top bar shows when audio was last synced (amber after 15 minutes) and the age of the oldest YouTube snapshot in view. The Spotify card and column appear only once an export has been imported. Source definitions are expandable, and audio and YouTube are never summed. On phones the episode column stays pinned while numbers scroll.
